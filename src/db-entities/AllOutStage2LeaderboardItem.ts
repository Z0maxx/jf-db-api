...
export const AllOutStage2LeaderboardItemSchema = defineEntity({
  ...
  properties: {
    ...
    participant: () => p.manyToOne(AllOutParticipant).ref().updateRule('restrict').deleteRule('cascade').index('fk_all_out_stage_2_leaderboard_participant_id'),
    map: () => p.manyToOne(AllOutStage2Map).ref().updateRule('restrict').deleteRule('cascade').index('idx_all_out_stage_2_leaderboard'),
  },
});
