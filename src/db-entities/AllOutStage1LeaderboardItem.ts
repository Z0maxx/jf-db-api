...
export const AllOutStage1LeaderboardItemSchema = defineEntity({
  ...
  properties: {
    ...
    participant: () => p.manyToOne(AllOutParticipant).ref().updateRule('restrict').deleteRule('cascade').index('fk_all_out_stage_1_leaderboard_participant_id'),
    map: () => p.manyToOne(AllOutStage1Map).ref().updateRule('restrict').deleteRule('cascade').index('idx_all_out_stage_1_leaderboard'),
  },
});
