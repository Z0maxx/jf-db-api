...
export const BountyPrizeSchema = defineEntity({
  ...
  properties: {
    ...
    division: () => p.manyToOne(Division).ref().updateRule('restrict').deleteRule('restrict').index('fk_bounty_prize_division_id'),
    event: () => p.manyToOne(BountyEvent).ref().updateRule('restrict').deleteRule('cascade').index('idx_bounty_prize'),
