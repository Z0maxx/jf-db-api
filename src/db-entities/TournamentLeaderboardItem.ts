...
export const TournamentLeaderboardItemSchema = defineEntity({
  ...
  uniques: [{ name: "unq_tournament_leaderboard", properties: ["map", "participant"] }],
  checks: [{ name: "chk_tournament_leaderboard_stage", expression: "`stage` > 1" }],
  properties: {
    ...
    participant: () =>
      p
        ...
        .updateRule("restrict")
        .deleteRule("cascade")
        .index("fk_tournament_leaderboard_participant_id"),
    map: () =>
      p
