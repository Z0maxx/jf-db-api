...
export const AllOutStage3LeaderboardItemSchema = defineEntity({
  ...
  properties: {
    ...
    participant: () => p.manyToOne(AllOutParticipant).ref().updateRule('restrict').deleteRule('cascade').index('fk_all_out_stage_3_leaderboard_participant_id'),
    map: () => p.manyToOne(AllOutStage3Map).ref().updateRule('restrict').deleteRule('cascade').index('idx_all_out_stage_3_leaderboard'),
  },
});
